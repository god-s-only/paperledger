...
class AuthRepositoryImpl @Inject constructor(private val alpacaApi: AlpacaApiService, private val paperLedgerSession: PaperLedgerSession, private val dao: PaperledgerDAO): AuthRepository {
    ...
    override fun getAccountInfo(accountId: String): Flow<Result<AccountInfo>> {
        val local = dao.observeAccountInfo().mapLatest { entity ->
            if (entity != null) Result.success(entity.toDomain())
            else Result.failure(AppError.EmptyBody)
        }
        val remote = flow {
            try {
                    ...
                    if (!response.isSuccessful) {
                        val errorMessage = response.errorBody()?.string()?.let {
                            runCatching { JSONObject(it).getString("message") }.getOrDefault("Error fetching account info")
                        ...
                        dao.replaceAccountInfo(body.toEntity())
                        emit(Result.success(body.toAccountInfo()))
                        Log.d("AccountInfo", "Refreshed")
                    }
                }
            catch (e: Exception) {
